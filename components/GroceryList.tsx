...
interface GroceryListProps {
  items: Item[];
  addItem: (newItem: string, e: FormEvent) => Promise<void>;
  deleteItem: (id: string) => void;
  sendAllItems: (itemsToSend: Item[]) => Promise<void>;
  loading: boolean;
}
        ...
        <ul>
          {items.map((item) => {
            return (
              <li
                key={item.id}
                className="group flex justify-between items-center text-lg py-2 pl-4 border-b border-gray-200"
              >
                {item.title}
                <RiDeleteBin5Line
                  onClick={() => deleteItem(item.id)}
                  className="mr-2 cursor-pointer invisible group-hover:visible"
                />
              </li>
            );
          })}
